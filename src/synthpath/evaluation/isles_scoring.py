...
def dice_coef_by_weighted_assignment(truth, prediction, batchwise=False):
    '''
    Performs lesion matching between the predicted lesions and the true lesions. A weighted bipartite graph between
    the predicted and true lesions is constructed, using dice as the edge weights. The returned value is the
    mean dice across predictions normalized by the number of lesions in the ground truth. Values close to 1
    indicate that the right number of lesions have been identified and that they overlap. Lower values indicate either
    the wrong number of predicted lesions or that they do not sufficiently overlap with the ground truth.
    Parameters
    ----------
    ...
    for idx_sample in range(truth_shape[0]):
        ...
        for idx_pred in range(num_pred_lesions):
            ...
            for idx_truth in range(num_truth_lesions):
                truth_lesion = truth_reshape == idx_truth
                cost_matrix[idx_pred, idx_truth] = dice_coef(truth_lesion, pred_lesion)
        row_ind, col_ind = linear_sum_assignment(cost_matrix=cost_matrix, maximize=True)
        total_precision = cost_matrix[row_ind, col_ind].sum()
        lcwa.append(total_precision / num_truth_lesions)
...
def precision(truth, prediction, batchwise=False):
    ...
    if(not batchwise):
        ...
        return precision_score(truth.reshape((num_pred,)), prediction.reshape((num_pred,)), zero_division=1)
    else:
...
def _recall(truth, prediction, batchwise=False, pos_label=1):
    ...
    if(not batchwise):
        # Convert to vector
        num_pred = np.prod(prediction.shape)
        return recall_score(truth.reshape((num_pred,)), prediction.reshape((num_pred,)), pos_label=pos_label, zero_division=1)
    else:
        # Need to get the precision for each sample in the batch
        recall_list = []
        num_pred = np.prod(prediction.shape[1:])
...
def _lesion_metrics_score(truth, prediction, empty_value=1.0):
    """
    Computes the lesion-wise metrics between two masks. Masks are considered true positives if at least one voxel
    overlaps between the truth and the prediction.
    ...
    tp, fp, fn = 0, 0, 0
    f1_score = empty_value
    precision_score = empty_value
    sensitivity_score = empty_value
    ...
    denom = tp + fp
    if(denom != 0):
        precision_score = tp / denom  
        
    # Compute sensitivity_score
    denom = tp + fn
    if(denom != 0):
        sensitivity_score = tp / denom
    
    return f1_score, precision_score, sensitivity_score